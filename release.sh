#!/bin/bash
# release.sh - Build and release static Go binaries
#
# Reads .binaries file (format: path:appname), builds for multiple platforms,
# injects build metadata, commits binaries, creates git tags, and pushes to GitHub.
#
# Usage: bash release.sh
#
# Features:
# - Static builds (CGO_ENABLED=0, -tags netgo)
# - Stripped binaries (-ldflags="-s -w")
# - Build metadata injection (commit, date, URL, license)
# - Automatic version detection from binary --version output
# - Git tag creation per binary (e.g., certmania-v1.2.0)
